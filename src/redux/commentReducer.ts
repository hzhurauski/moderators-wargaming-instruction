...
export const actions = {
    setComments: (comments: Array<CommentType>) => ({ type: 'set comments', comments } as const),
    createNewComment: (comment: CommentType) => ({ type: 'create new comment', comment } as const),
    changeMessage: (message: string) => ({ type: 'current message', message } as const),
    commentSubmitting: (submitting: boolean) => ({ type: 'submitting', submitting } as const)
}
